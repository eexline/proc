...
function requestApiPath(req: VercelRequest): string {
  const raw = req.url ?? "/";
...
export async function proxy(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const apiPathname = requestApiPath(req);
